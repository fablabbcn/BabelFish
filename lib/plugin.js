...
// WARNING: It is assumed tha document.body is loaded to create a
// plugin.
function str2ab(msg) {
  var buf = new ArrayBuffer(msg.length); // 2 bytes for each char
...
function dbg(msg) {
  console.log.apply(console, ["[plugin] "].
...
Serial.prototype = {
  ...
  getDevices: function (callback) {
    this.imm_(function () {
      ...
      // XXX: ADHOC: Devices with no vid/pid pair are certainly not
      // arduinos.
      callback(ports.filter(function (d) {
	return (d.description.indexOf('=') >= 0);
      }).map(function (d) {
  ...
  connect: function (path, options, callback) {
	...
	plugin.serialRead(
	  path, options.bitrate,
	  function (from, msg) {
	    console.assert(from.instance_id == plugin.instance_id,
			   "Receved from strange plugin " +
			   from.instance_id + " != " + plugin.instance_id);
	    console.log("Received bytes: " + msg);
	    if(msg.length && first_call) {
	      this.onReceive.dispatch({connectionId: plugin.instance_id,
				       data: str2ab(msg)});
	    }
	    // XXX: The plugin throws garbadge as the first
	    // message. Ignore it.
	    first_call = true;
	  }.bind(this),
	  function (from, msg) {
	    this.onReceiveError.dispatch({connectionId: plugin.instance_id,
					  data: str2ab(msg)});
	  }.bind(this));
      } else
  ...
  send: function (connectionId, data, callback) {
	...
	throw Error("Serial tried to send msg of type " + typeof data +
      ...
      var bytes = plugin.write(msg);
      console.log("[plugin] Sent bytes: " + bytes + "/" + msg.length);
      callback({bytesSent: bytes});
    });
  },
