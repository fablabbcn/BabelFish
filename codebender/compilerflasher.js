compilerflasher = function (loadFiles) {
    ...
    this.pluginHandler = new function () {
        ...
        this.doflashBootloader = function (programmer, board) {
                ...
                if (window.chrome && !debugBabelfish) {
        ...
        this.connect = function () {
            ...
            var speed = $baudRates.find('option:selected').val();
                ...
                if (typeof window.isWindowsAndChrome != 'undefined' && window.isWindowsAndChrome()) {
                        $serialHud.append(escapeHtml('Warning! Because of a known bug on Chrome, you should press the disconnect button before physically disconnect your device.\nOr else the connection with the serial monitor will stay open until you restart your browser.\n\n'));
                }
        ...
        this.disconnect = function (notified) {
                if (window.operationInProgress || !this.connected) {
                        return;
                }
        ...
        this.serialHudAppend = function(line) {
                var $serialHud = $('#serial_hud');
            if (isNaN(line)) {
                serialHudWrite($serialHud.html() + line + "<br>");
            } else {
                if (line == "13") {return;}
        ...
        this.serialSend = function () {
                ...
                if ($('#echo_check').is(':checked')) {
                        $("#serial_hud").append('<span class="serial-monitor-echo">'+escapeHtml(inputText)+'</span>');
                        if (breakFlag) {
                                $("#serial_hud").append('<br>');
                        }
                }
        };
